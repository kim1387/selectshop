...
function execSearch() {
     ...
     */
    $("#search-result-box").empty()     // 이전 결과값 지움
    // 1. 검색창의 입력값을 가져온다.
    let query = $('#query').val();
    ...
    $.ajax({
        ...
        success: function (response) {        //response: ItemDto의 list(Controller에 해놓음)
            for (let i = 0; i < response.length; i++) {
                let itemDto = response[i];
                let tmpHtml = addHTML(itemDto); // 아래에 있음. itemDto를 html에 넣어줌
...
function addHTML(itemDto) {
     ...
     * image, title, lprice, addProduct 활용하기 -> itemDto에 있음
    ...
    return `<div class="search-itemDto">
                ...
                <div class="search-itemDto-right">
                    ...
                    <!--add product: 상품 등록하는 함수 실행.-->
                    <!--ItemDto : JSON이니까 스트링으로 변경해줌-->
                </div>
            </div>`
}
